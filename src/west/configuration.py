...
class Configuration:
    ...
    def __init__(self, topdir: PathType | None = None):
        ...
        self._local_paths = [Path(p) for p in _location(ConfigFile.LOCAL, topdir, find_local=False)]
        self._global_paths = [Path(p) for p in _location(ConfigFile.GLOBAL, topdir)]
        self._system_paths = [Path(p) for p in _location(ConfigFile.SYSTEM, topdir)]
...
def _ensure_single_config(topdir):
