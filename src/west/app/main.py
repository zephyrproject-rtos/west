...
class WestApp:
    ...
    def load_aliases(self):
        ...
        for item in self.config.items():
            ids = item[0].split('.', 1)
            if ids[0] == 'alias':
                self.aliases[ids[1]] = Alias(ids[1], item[1])
    ...
    def run_command(self, argv, early_args):
        ...
        if not early_args.help and early_args.command_name != "help":
            ...
            while early_args.command_name in aliases:
                new_args = shlex.split(aliases[early_args.command_name].args)
                # Make sure we don't end up in an infinite loop
                del aliases[early_args.command_name]
                self.queued_io.append(lambda cmd: cmd.dbg(
                    f'Replacing alias {early_args.command_name} with {new_args}'
                ))
...
class Alias(WestCommand):
    ...
    def __init__(self, cmd, args):
        super().__init__(cmd, f'"{args}"', f'An alias that expands to "{args}"')
