...
class Init(_ProjectCommand):
    def __init__(self):
        super().__init__(
            ...
            f'''\
Initialize a west workspace (topdir) by creating a `.west` directory and
setting option `manifest.path` in the local configuration file `.west/config`.
The option is set to the directory (relative to topdir) that contains a west
manifest `west.yml`.
The manifest can originate either from a remote repository URL (which is cloned
into a folder in topdir) or from an already existing local directory.
...
folder in the topdir. The repositoy has to contain a west manifest.
...
existing local `manifest_directory`. It defaults to current working
directory.

The west workspace (topdir) will be the `manifest_directory`'s parent if no
other `--topdir` is provided.
...
''',
        ...
        parser.add_argument(
            ...
            help='''With --local: the path to the local manifest repository
                    ...
                    --topdir is provided), or the subfolder where west will
                    clone all projects.''',
        )
