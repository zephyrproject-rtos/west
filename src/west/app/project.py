...
---------------------------------------
West clones the given remote repository (provided via `-m / --manifest-url`).
Config option `manifest.path` is set to point to this git clone.
...
initialization (e.g. by default west clones to <directory>/zephyr/.git).
...
to point to the `manifest_directory`.
...
''',
  ...
  init around existing local manifest:
...
''',
        ...
        parser.add_argument(
            ...
            help='''the location of the west workspace where .west directory
        ...
        parser.add_argument(
            ...
            help='''With --local: the path to a local directory which contains
                    ...
                    Otherwise (if -m is used):
                    the location where west will clone the remote manifest
                    ''',
        )
