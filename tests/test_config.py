...
def test_global_creation():
    ...
    update_config('pytest', 'key', 'val', configfile=GLOBAL)
...
def test_local_creation():
    ...
    update_config('pytest', 'key', 'val', configfile=LOCAL)
...
def test_local_creation_with_topdir():
    ...
    update_config('pytest', 'key', 'val', configfile=LOCAL, topdir=str(topdir))
    assert not system.exists()
    assert not glbl.exists()
    assert not local.exists()
...
def test_delete_basic():
    ...
    with pytest.raises(KeyError):
        delete_config('pytest', 'key', configfile=SYSTEM)
    with pytest.raises(KeyError):
        delete_config('pytest', 'key', configfile=GLOBAL)
    delete_config('pytest', 'key', configfile=LOCAL)
...
def test_delete_all():
    ...
    delete_config('pytest', 'key', configfile=ALL)
    assert 'pytest' not in cfg(f=ALL)
...
def test_delete_local_one():
    ...
    update_config('pytest', 'key1', 'foo', configfile=LOCAL)
    update_config('pytest', 'key2', 'bar', configfile=LOCAL)
    delete_config('pytest', 'key1', configfile=LOCAL)
