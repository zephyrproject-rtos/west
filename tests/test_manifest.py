...
def test_import_map_error_handling():
    ...
    # values that are maps. The error messages come from the rust
    # binding's serde-derived deserialiser (deny_unknown_fields + the
    # custom OneOrMany visitor); they convey the failure mode and the
    # expected type. v1's exact wording ("invalid import contents",
    # "bad import <key>") isn't preserved — the messages here are
    # more descriptive in exchange.
    ...
    def check_error(import_map, *expected_substrings):
        with pytest.raises(MalformedManifest) as e:
    ...
    # Unexpected keys: the message names the bad key and lists the
    # valid ones — serde's `deny_unknown_fields` output.
    check_error({'invalid-key': 1}, 'unknown field', 'invalid-key', 'name-allowlist')
    # Wrong type on an allow/block-list field: OneOrMany's visitor
    # rejects with "invalid type: map, expected a value or a list of values".
    check_error({'name-allowlist': {}}, 'invalid type: map', 'list of values')
    check_error({'path-allowlist': {}}, 'invalid type: map', 'list of values')
    check_error({'name-blocklist': {}}, 'invalid type: map', 'list of values')
    check_error({'path-blocklist': {}}, 'invalid type: map', 'list of values')
    # Wrong type on the scalar `path-prefix:` field: serde derive's
    # own "invalid type: map, expected a string".
    check_error({'path-prefix': {}}, 'invalid type: map', 'expected a string')
...
def test_import_loop_detection_self(manifest_repo):
    ...
    #
    # The legacy resolver had no real cycle detection — it relied on
    # python recursing into itself until a RecursionError fired, then
    # repackaged that as `_ManifestImportDepth` (the original commit
    # called itself "hacky import loop detection"). That hack was the
    # reason the legacy test parametrized over `sys.setrecursionlimit`
    # and skipped on cpython 3.13.8-3.13.12 (where the RecursionError
    # behavior shifted). The rust resolver tracks visited files in a
    # HashSet and reports the cycle directly via `ManifestImportFailed`
    # before depth is ever exceeded, so none of that machinery applies.
    ...
    with pytest.raises(ManifestImportFailed, match=r'import cycle'):
        MF()
...
def test_invalid_groups():
    ...
    # Scalar values for a list-typed field surface as the parser's
    # natural type-mismatch error. v1 got pykwalify's "is not a list";
    # the rust port routes through serde + saphyr which describes the
    # same shape as "expected sequence" instead.
    for fmt in [fmt_scalar_project, fmt_scalar_group_filter]:
        check(fmt, 'hello', 'expected sequence')
        check(fmt, 3, 'expected sequence')
        check(fmt, 3.14, 'expected sequence')


def test_groups():
    # Basic test for valid project groups. v1 silently coerced numeric
    # YAML scalars to strings; the rust port drops that YAML wart and
    # requires entries to be quoted (callers add a one-keystroke
    # quote when they want numeric-looking names).
    ...
    # v2 changes
    assert not is_group(1)
    assert not is_group(3.14)
...
def test_invalid_manifest_group_filters():
    ...
    # v1 got pykwalify's "not a list"; the rust port surfaces serde's
    # natural "expected a sequence" for the same shape.
    check2('hello', 'expected a sequence')
    check2(3, 'expected a sequence')
    check2(3.14, 'expected a sequence')
...
def test_group_filter_project_import(manifest_repo):
    ...
    # v0.9's "imports do not contribute to the manifest's group-filter"
    # quirk is intentionally not preserved in the rust port; only the
    # v0.10 contract (which has been the default for years) is supported.
...
def test_group_filter_self_import(manifest_repo):
    ...
    # Same as test_group_filter_project_import: v0.9's import-suppression
    # quirk is not preserved.
...
def test_group_filter_imports(manifest_repo):
    ...
    #
    # v1 also documented a v0.9 schema version where imports did not
    # contribute to the manifest's group-filter (and set a
    # `_legacy_group_filter_warned` attribute on the Manifest). The
    # rust port drops that compat path; only v0.10 semantics remain.
